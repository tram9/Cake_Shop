					...
					<div class='product-container' onclick="Display_PrdDetail('<?php echo $data[$i]['masp'] ?>')">
						<a data-toggle='modal' href='product/PrdDetail/<?php echo $data[$i]['masp'] ?>' data-target='#modal-id'>
							<div style="text-align: center;" class='product-img'>
								<img src='public/images/<?php echo $data[$i]['anhchinh'] ?>'>
							</div>
...
<script type="text/javascript">
	...
	switch(cr){
		case 'onsale':
			$('#dgg').css('background-color','rgb(255, 205, 205)');
			break;
		case 'newest':
			$('#spm').css('background-color','rgb(255, 205, 205)');
			break;
		case 'bestselling':
			$('#mntq').css('background-color','rgb(255, 205, 205)');
			break;
	}
</script>