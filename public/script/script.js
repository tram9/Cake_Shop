...
function Display_PrdDetail1(matt){
  $('#modal-id').attr('data-remote','tintuc/PrdDetail1/'+matt);
  $('#modal-tintuc').empty();

  $.ajax({
    url : "tintuc/PrdDetail1/"+matt,
    type : "post",
    dataType:"text",
    data : {
      matt
...
function loadmore(start){
  ...
  $.ajax({
   ...
   success : function (result){
    if(!result){alert("Đã hết dữ liệu để hiển thị!"); return 0;}
